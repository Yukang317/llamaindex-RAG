...
// 响应式变量：绑定到页面的开关/下拉框/滑块
const message = ref('');        // 用户输入的消息
const model = ref("qwen-turbo-2025-04-28"); // 默认模型
const temperature = ref(0.7);   // 温度（控制AI回复随机性）
const maxTokens = ref(2000);    // 最大Token（控制回复长度）
const stream = ref(false);      // 流式输出开关

// 定义事件：给父组件（ChatView）传值
 ...
 * 核心逻辑：
 * 这个组件是 “输入面板”，负责：
1. 让用户输入消息、选择模型、调温度 / Token、开 / 关流式输出；
2. 点击 “发送” 按钮 → 触发sendMessage → 给父组件（ChatView）发send事件，把所有参数传过去；
3. 父组件接收到事件后，调用chatStore.sendMessage真正发请求。
 */
